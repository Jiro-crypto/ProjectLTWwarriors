...
namespace ProjectLTWwarriors.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult LandingPage(string searchTerm, int? page)
        {
            ...
            // Lấy products từ DB + include Category + ProductImages
            var products = _db.Products
                             .Include(p => p.Categories)
                             .Include(p => p.ProductImages)
                             .Include(p => p.OrderItems)
                             .Where(p => p.Status == "active") //lấy những sản phẩm còn hoạt động
                             .AsQueryable();
            ...
            // SẢN PHẨM NỔI BẬT – top 8 sản phẩm có nhiều OrderItems
            model.FeaturedProducts = products
                .OrderByDescending(p => p.OrderItems.Count()) // IMPORTANT: Đồ án dùng OrderItems
                .Take(8)
                .ToList();

            // SẢN PHẨM MỚI – phân trang theo CreatedAt (hoặc Id)
            model.NewProducts = products
                .OrderByDescending(p => p.CreatedAt) // đồ án có CreatedAt
